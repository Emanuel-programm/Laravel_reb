        ...
        <div class="flex justify-between items-center">
          ...
          <div class="flex space-x-3 ml-4">
            ...
            <form method="POST" action="{{ route('jobs.destroy',$job->id) }}"
              onsubmit="return confirm('Are you sure do you want to delete this listing?')"
              >
              @csrf
              @method('DELETE')
              <button
...
<div x-data="{ open: false }" id="applicant-form">
  ...
  >
    <div @click.away="open = false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md  max-h-[90vh] overflow-y-auto">
      ...
      <form method="POST" enctype="multipart/form-data" action="{{ route('applicants.store',$job->id)}}">
        @csrf
