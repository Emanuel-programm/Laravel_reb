...
<div class="rounded-lg shadow-md bg-white p-4">
  <div class="flex items-center gap-4">
    @if($job->company_logo)
      <img src="/storage/{{ $job->company_logo }}"
           alt="{{ $job->company_name }}" class="w-14" />
    @endif
    <div>
  ...
  <!-- Str::limit() cuts description at 100 chars -->
  ...
  <ul class="my-4 bg-gray-100 p-4 rounded">
    ...
    <li><strong>Location:</strong> {{ $job->city }}, {{ $job->state }}
      @if($job->remote)
    ...
    <li><strong>Tags:</strong> {{ ucwords(str_replace(',', ', ', $job->tags)) }}</li>
  </ul>

  <a href="{{ route('jobs.show', $job->id) }}"
     class="block w-full text-center px-5 py-2.5 rounded border
